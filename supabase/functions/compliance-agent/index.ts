...
// Ingest -> Retrieve -> Verify -> Generate -> Attach & flag -> Route for approval
//
// Mirrors the worked example in the Oodles V2 concept doc:
//   "Do you encrypt customer data at rest?"
//   -> find the policy, verify live config, locate prior audit evidence,
//      identify the control owner, check freshness, generate the answer,
//      attach evidence, flag gaps.
...
import { getConnector } from "../_shared/connectors/base.ts";
import { ComplianceQuestion, ComplianceRequestType, EvidenceNode } from "../_shared/types.ts";
...
async function answerQuestion(
  ...
  // answer from `candidates` + `verifications`, with inline citations.
...
async function retrieveCandidateEvidence(orgId: string, questionText: string): Promise<EvidenceNode[]> {
...
async function routeForApproval(orgId: string, actorId: string | null, questionId: string) {
...
interface ComplianceAgentRequest {
  action: "create-request" | "answer" | "status" | "route-approval";
